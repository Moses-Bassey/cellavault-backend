...
module.exports = {
  up: async (queryInterface) => {
    ...
    if (
      ...
      console.log(
        '⚠️  No countries found. Please run countries seeders first.',
      );
      return;
    }
    ...
    for (const countryName in allStatesByCountry) {
      ...
      if (!countryId) {
        console.log(
          `⚠️  Country "${countryName}" not found in database, skipping states`,
        );
        continue;
      }
    ...
    if (allStatesToInsert.length > 0) {
      try {
        ...
        for (let i = 0; i < allStatesToInsert.length; i += batchSize) {
          ...
          console.log(
            `✅ Inserted batch: ${inserted}/${allStatesToInsert.length} states`,
          );
        }

        console.log(
          `✅ Successfully inserted ${allStatesToInsert.length} states for all countries`,
        );
      } catch (error) {
