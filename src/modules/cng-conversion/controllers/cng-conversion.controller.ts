...
export class CngConversionController {
  ...
  async create(
    ...
    return ResponseUtil.handleResponse(
      data,
      'CNG conversion request created successfully',
      HttpStatus.CREATED,
    );
  }
  ...
  async fetchUserCngConversionsStats(
    ...
    const userId = Validators.validateUuid(req.user.userId);
    const data =
      await this.cngConversionService.fetchUserCngConversionsStats(userId);
    return ResponseUtil.handleResponse(
      data,
      'Fetch cng conversions retrieved successfully',
      HttpStatus.OK,
