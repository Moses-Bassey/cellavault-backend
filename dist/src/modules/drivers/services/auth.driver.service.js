...
let AuthDriverService = class AuthDriverService {
    ...
    async createAccount(input) {
        try {
            ...
            return response_utils_1.ResponseUtil.success(data, 'User created successfully', common_1.HttpStatus.CREATED);
        }
        catch (error) {
            console.log(error);
    ...
    async resetPassword(input) {
        ...
        const data = await this.driverRepository.update(user.id, { password: await password_util_1.PasswordUtil.hashPassword(password) });
